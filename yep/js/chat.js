...
function addListeners() {
    ...
    function handleMessage(channel, userstate, message, fromSelf) {
        if ($("div.chat-box").length > 3) {
            $('.chat-box div:nth-child(1)').remove();
        }
...
function showMessage({ chan, type, message = '', data = {}, timeout = 0, attribs = {} } = {}) {
    ...
    chatEle.append(chatLine_);

    setTimeout(() => chatLine_.classList.add('visible'), 100);

    if (chatEle.children().length > 100) {
        chatEle.children()[0].remove()
    }
    ...
    if (timeout) {
        setTimeout(() => {
            if (chatLine_.parentElement) {
                ...
                setTimeout(() => chatEle.remove(chatLine_), 1000);
            }
        }, timeout);
    }
}
